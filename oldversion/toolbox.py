...
class toolbox:
    ...
    def test(self, X, a1):
        if type(a1) == str:
            ...
            if config.evalMetric == 'tok.acc':
                scoreList = decode_tokAcc(X, self.Model)
            elif config.evalMetric == 'str.acc':
                scoreList = decode_strAcc(X, self.Model)
                ...
                scoreList = decode_fscore(X, self.Model)
            ...
            config.swOutput = open(outfile, 'w')
            if config.evalMetric == 'tok.acc':
                scoreList = self.decode_tokAcc(X, self.Model)
            elif config.evalMetric == 'str.acc':
                scoreList = self.decode_strAcc(X, self.Model)
            elif config.evalMetric == 'f1':
                scoreList = self.decode_fscore(X, self.Model)
            else:
                raise Exception('error.')
            config.swOutput.close()
            return scoreList
    ...
    def decode_fscore(self, X, m):
        X2 = []

        self.multiThreading(X, X2)
        goldTagList = []
        resTagList = []
        for x in X2:
            res = ''
